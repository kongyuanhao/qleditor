...
class CreateProject(object):
    ...
    def __init__(self, app):
        ...
        self.tables = self.app.tables.all()
        self.configs = self.get_configs()
        local("cd %s" % settings.LOCAL_PROJECTS_PATH)
    def create_code(self):
        if not os.path.exists(self.project_path):
            # 创建项目svn 并进入
            local("svn mkdir %s" % self.project_name)
            local("cd %s" % self.project_name)
            # write manage.py
            self.write_file(
        ...
        local("svn add . --force" % self.project_name)
        local("svn commit -m 'abcdefghijklmn'")
    ...
    def write_models_file(self):
        ...
        for table in self.tables:
            ...
            for field in table.fields.all():
                ...
                tables[table.name].append(field_str)
