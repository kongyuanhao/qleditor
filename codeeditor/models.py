...
class AppModel(models.Model):
    ...
    svn_path = models.CharField(verbose_name=u"svn地址",max_length=255,blank=True,null=True)
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        if not self.id and not self.name and self.des:
            self.name = pinyin.get_initial(self.des).replace(" ", "") + "_" + shortuuid.ShortUUID().random(3)
        if not self.server:
...
class FieldModel(models.Model):
    ...
    table = models.ForeignKey(TableModel, related_name="fields", blank=True, null=True)
